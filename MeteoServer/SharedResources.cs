...
namespace ASCOM.QAstroMeteo
{
    ...
    public static class SharedResources
    {
        ...
        private static Telnet s_sharedTelnet = new Telnet();                  // Shared telnet connection
        ...
        private static int TELNET_RESPONSE_WAIT = 1000;   //Number Miliseconds to wait after sending data to collect a response.
        ...
        private static int iTimerInterval = 11000;          //Milliseconds = 11 seconds
        ...
        private static Telnet SharedTelnet { get { return s_sharedTelnet;} }
        ...
        public static string SendMessage(string function, string message)
        {
            ...
            lock (lockObject)
            {
                ...
                if (testConnected && !String.IsNullOrEmpty(message))
                {
        ...
        private static bool testConnected
        {
            get
            {
                if (ASCOM.QAstroMeteo.Properties.Settings.Default.SerialConnection)
        ...
        private static string SendMessageTelnet(string msg)
        {
            ...
            while ((noDataCount < 3) && (strRec.Length == 0))
            {
                SharedTelnet.WriteLine(msg);
                Thread.Sleep(TELNET_RESPONSE_WAIT);
                strRec = SharedTelnet.Read();
                noDataCount++;
            }
            return strRec;
        }
                ...
                lock (lockObject)
                {
                    if (value)
                    {
                        if (s_z == 0)
                        {
                            ...
                            if (ASCOM.QAstroMeteo.Properties.Settings.Default.SerialConnection)
                                tempConnected = ConnectSerial();
                            else
                                tempConnected = ConnectTelnet();
                    ...
                    else
                    {
                        ...
                        {
                            if (ASCOM.QAstroMeteo.Properties.Settings.Default.SerialConnection)
                                SharedSerial.Connected = value;
                            else
                                SharedTelnet.Connected = value;
        ...
        #region Private Connect Methods

        private static bool ConnectSerial()
        {
            try
            {
        ...
        private static bool ConnectTelnet()
        {
            try
            {
                SharedTelnet.HostName = ASCOM.QAstroMeteo.Properties.Settings.Default.Server;
                SharedTelnet.PortNumber = 23;
                SharedTelnet.Connected = true;

                if (!SharedTelnet.Connected)
                {
                    MessageBox.Show("Q-Astro Meteo device not detected on IP " + SharedTelnet.HostName, "Device not detected", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    SharedResources.tl.LogMessage("Connected answer", "Wrong answer ");
                }
            }
            catch (SocketException exception)
            {
                MessageBox.Show("Q-Astro Meteo Telnet connection not established for " + SharedTelnet.HostName, "Invalid server state", MessageBoxButtons.OK, MessageBoxIcon.Error);
                SharedResources.tl.LogMessage("Server connection not estalished", exception.Message);

            }
            catch (System.IO.IOException exception)
            {
                MessageBox.Show("Q-Astro Meteo Telnet port not opened for " + SharedTelnet.HostName, "Invalid server state", MessageBoxButtons.OK, MessageBoxIcon.Error);
                SharedResources.tl.LogMessage("Server connection not estalished", exception.Message);
            }
            catch (System.UnauthorizedAccessException exception)
            {
                MessageBox.Show("Q-Astro Meteo Access denied to server " + SharedTelnet.HostName, "Access denied", MessageBoxButtons.OK, MessageBoxIcon.Error);
                SharedResources.tl.LogMessage("Access denied to server", exception.Message);
            }
            catch (ASCOM.DriverAccessCOMException exception)
            {
                MessageBox.Show("Q-Astro Meteo ASCOM driver exception: " + exception.Message, "ASCOM driver exception", MessageBoxButtons.OK, MessageBoxIcon.Error);
                SharedResources.tl.LogMessage("ASCOM driver exception", exception.Message);
            }
            catch (System.Runtime.InteropServices.COMException exception)
            {
                MessageBox.Show("Q-Astro Meteo Connection read timeout for server " + SharedTelnet.HostName, "Timeout", MessageBoxButtons.OK, MessageBoxIcon.Error);
        ...
        private static void setLuxValue()
        {
            string lValues = "us";
            lValues += "LUX" + ASCOM.QAstroMeteo.Properties.Settings.Default.Lux;

            SendMessage(ASCOMfunction, lValues);
        }
